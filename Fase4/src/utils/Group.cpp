...
void Rotate::apply() {
    if (isAnimated && time > 0) {
        ...
        float currentAngle = (elapsed * 360.0f) / time; // 360 graus no tempo especificado
...
void Translate::apply() {
    if (time > 0 && p.size() >= 4) { // Requer mínimo de 4 pontos 
        float pos[3], deriv[3];
        float elapsed = glutGet(GLUT_ELAPSED_TIME) / 1000.0f;
        float gt = fmod(elapsed, time) / time; // t entre 0 e 1
